...
if ((day_of_week == 6) | (day_of_week == 7)):  # проверка является ли день недели субботой или воскресеньем
    sum = duration * cost_Per_Minute * (1 - discount)  # расчет стоимости, если сегодня выходной
else:
    sum = duration * cost_Per_Minute  # расчет стоимости, если сегодня не выходной
# ctypes.windll.user32.MessageBoxW(0, 'Стоимость разговора: ' + str(sum) + ' ₽', 'Стоимость телефонного разговора', 0)
messagebox.showinfo(title = 'Стоимость разговора', message = 'Стоимость разговора: ' + str(sum) + ' ₽')  # вывод стоимости в окно вывода
